...
```bash
stepfunctions-enrichment$ sam build --use-container
```
...
```bash
stepfunctions-enrichment$ sam logs -n StockCheckerFunction --stack-name stepfunctions-enrichment --tail
```
...
```bash
stepfunctions-enrichment$ pip install -r tests/requirements.txt --user
# unit test
stepfunctions-enrichment$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
stepfunctions-enrichment$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name stepfunctions-enrichment
```
